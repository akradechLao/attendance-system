...
const employees = [
  ...
  { name: "จิรายุ เริงหาญ", groupType: "B" as const, wfhQuota: 0, preferredOffDay: null },
...
async function main() {
  ...
  await prisma.$executeRawUnsafe(
    `DELETE FROM admin_users WHERE username = 'admin' AND id NOT IN (
      SELECT MIN(id) FROM admin_users WHERE username = 'admin'
    )`
  );
